# Java Notes

## 语言层面

- [枚举](./src/main/java/basic/enums)
- [可变参数](./src/main/java/basic/varargs)
- [注解](./src/main/java/basic/annotation)
- [反射](./src/main/java/basic/reflection)
- [泛型](./src/main/java/basic/generics)
- [Lambda 表达式](./src/main/java/basic/lambda_expressions)
- [函数式接口](./src/main/java/basic/functional_interface)
- [聚合操作](./src/main/java/basic/aggregate_operations)
- [集合](./src/main/java/basic/collection)
- [并发](./src/main/java/basic/concurrency)
- [异常](./src/main/java/basic/exception)
- [IO](./src/main/java/basic/io)
- [接口](./src/main/java/basic/interfaces)
- [网络](./src/main/java/basic/network)

## 编程范式

- [AOP](./src/main/java/aop)
- [Java 中的函数式编程](./src/main/java/functional_programming)

## 数据结构

- [布隆过滤器](./src/main/java/data_structure/bloom_filter)

## 算法

- [LRU](./src/main/java/algorithm/lru)
- [回溯](./src/main/java/algorithm/backtracking)
- [动态规划](./src/main/java/algorithm/dp)
- [分治法](./src/main/java/algorithm/dc)

### 排序

[排序知识点](./src/main/java/algorithm/sort)

 - [冒泡排序](./src/main/java/algorithm/sort/bubble)

## 数据存储

- [Database](./src/main/java/database)
- [Cache](./src/main/java/cache)

### 数据库

- [H2](./src/main/java/database/h2)
- [mysql](./src/main/java/database/mysql)
- [postgres](./src/main/java/database/postgresql)

## 工具和库

- [Lombok](./src/main/java/libraries/lombok)

## 测试

- [Mockito](./src/main/java/test/mockito)

## Spring

- [Spring Boot 2.x](./src/main/java/spring/boot)

## 设计模式

- [设计模式](./src/main/java/design/patterns)

## 正则

- [正则表达式](./src/main/java/regex)

## 领域驱动设计

- [Event Storming](./src/main/java/ddd/event_storming.md)

## IDE

- [IntelliJ IDEA](./src/main/java/idea)

## 全文检索

- [Elasticsearch](./src/main/java/elasticsearch)

## 架构

- [架构](./src/main/java/architecture)
